...
$(function() {
    ...
    if (!badIE) {
        ...
        wax.tilejson(urlBase + basemap + '.jsonp', function(baseTilejson) {
            ...
            b.setCenterZoom(new mm.Location(-5.018061, 32.826279), 6);
        });
    }
    ...
    wax.tilejson(url, function(tilejson) {
        ...
        m.setCenterZoom(new mm.Location(-5.018061, 32.826279), 6);
        m.setZoomRange(4, 10);
    ...
    function easeIt(x, y, z, callback) {
        ...
        easey().map(m)
            .to(m.locationCoordinate({
                lat: x,
                lon: y
            }).zoomTo(z)).path(path).run(time);
    }
    ...
    $('#sidebar .story p a, #sidebar .partners p a').click(function(e) {
        ...
        } else if ($(this).parent().hasClass('explore-data')) {
    ...
    function updateEmbedApi() {
        ...
        if (layer == '') {
            ...
            embedUrl = '<iframe src="' + urlBase + basemap + '/mm/zoompan,tooltips,legend,bwdetect.html#' + m.coordinate.zoom + '/' + center.lat + '/' + center.lon + '"' + ' frameborder="0" width="650" height="500"></iframe>';
        } else {
            apiUrl = urlBase + basemap + ',' + layer + '.jsonp';
            embedUrl = '<iframe src="' + urlBase + basemap + ',' + layer + '/mm/zoompan,tooltips,legend,bwdetect.html#' + m.coordinate.zoom + '/' + center.lat + '/' + center.lon + '"' + ' frameborder="0" width="650" height="500"></iframe>';
        }
